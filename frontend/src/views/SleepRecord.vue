<template>
  <div class="sleep-record">
    <el-card>
      ...
      <el-form :model="formData" label-width="100px" class="form-box">
        <el-form-item label="入睡时间">
          <el-input v-model="formData.bedTime" placeholder="HH:mm" />
        </el-form-item>
        <el-form-item label="起床时间">
          <el-input v-model="formData.wakeTime" placeholder="HH:mm" />
        </el-form-item>
        <el-form-item label="睡眠时长(小时)">
          <el-input-number v-model="formData.sleepDuration" :step="0.5" :min="0" :max="16" />
        </el-form-item>
        <el-form-item label="睡眠质量(1-10)">
          <el-slider v-model="formData.sleepQuality" :min="1" :max="10" />
        </el-form-item>
      ...
      <el-table :data="recordList" style="width: 100%; margin-top: 20px">
        ...
        <el-table-column prop="sleepQuality" label="睡眠质量" width="100">
          <template #default="scope">
            <el-rate v-model="scope.row.sleepQuality" :max="10" disabled />
          </template>
        </el-table-column>
    ...
    <!-- 编辑对话框 -->
    <el-dialog v-model="editVisible" title="编辑睡眠记录" width="400px">
      <el-form :model="editFormData" label-width="100px">
        <el-form-item label="入睡时间">
          <el-input v-model="editFormData.bedTime" />
        </el-form-item>
        <el-form-item label="起床时间">
          <el-input v-model="editFormData.wakeTime" />
        </el-form-item>
        <el-form-item label="睡眠时长">
          <el-input-number v-model="editFormData.sleepDuration" :step="0.5" />
        </el-form-item>
        <el-form-item label="睡眠质量">
          <el-slider v-model="editFormData.sleepQuality" :min="1" :max="10" />
        </el-form-item>
        <el-form-item label="备注">
...
const submitForm = async () => {
  try {
    await healthApi.addSleep({
      ...formData.value,
      userId: currentUser.value.id
    })
    ElMessage.success('添加成功')
    formData.value = { bedTime: '', wakeTime: '', sleepDuration: null, sleepQuality: 5, recordDate: new Date().toISOString().split('T')[0], notes: '' }
    loadRecords()
  } catch (error) {
...
const loadRecords = async () => {
  try {
    const response = await healthApi.getSleepList(
...
const saveEdit = async () => {
  try {
    await healthApi.updateSleep(editingId.value, editFormData.value)
